  ...
  return (
    <>
      ...
      <motion.nav
        ...
        onMouseLeave={() => {
          if (scrollY.get() > 50) setIsNavVisible(false);
        }}
        className={`fixed top-0 left-0 w-full h-14 md:h-16 z-[9998] pointer-events-auto grid grid-cols-3 items-center px-6 md:px-10 backdrop-blur-md transition-colors duration-300 ${isOverDarkSection ? 'bg-black/20' : 'bg-[#FFC5D0]/90'}`}
        style={{
      ...
      >
        ...
        <div className="relative z-30 flex justify-center items-center w-full transition-all duration-300 font-['Clarendon']">
          <PillNav
            items={isArabic ? [
        ...
        <div className="relative z-30 flex justify-end items-center gap-4">
          <button
            ...
            className={`relative z-30 border-2 px-4 py-1 rounded-full text-sm font-bold transition-colors duration-300 ${navControlClasses}`}
          >
            {isArabic ? 'EN' : '\u0639'}
          </button>
          ...
          <a
            ...
            className={`relative z-30 transition-colors duration-300 ${navIconClasses}`}
